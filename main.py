...
def new_trading_state():
    return {"is_active": False, "sl_hit_count": 0, "trade_history": [], "legs": {"CE": {}, "PE": {}}}
...
def start_strategy():
    ...
    # 1. 9:15 AM SENSEX Index Candle -> High/Low (దొరకకపోతే ఇక్కడే ఆగిపోతుంది)
    high_915, low_915 = fetch_915_high_low(broker, access_token)
    if high_915 is None or low_915 is None:
        return jsonify({
            "status": "error",
            "message": "Live 9:15 market data దొరకలేదు (weekend/holiday/invalid token కావొచ్చు). "
                        "Strategy మొదలుపెట్టలేదు — తప్పుడు ధరతో ట్రేడ్ కాకుండా ఆపాం."
        }), 503
    ...
    if ce_ref_price is None or pe_ref_price is None:
        return jsonify({
            "status": "error",
            "message": "Live 10:30 candle డేటా (CE/PE రెండింటికీ) దొరకలేదు. Strategy మొదలుపెట్టలేదు."
        }), 503
    ...
    return jsonify({
        ...
        "ce_entry_trigger": state["legs"]["CE"]["entry_trigger_price"],
        "ce_reference_source": ce_source,
        "pe_symbol": pe_symbol, "pe_atm_strike": pe_atm,
        "pe_entry_trigger": state["legs"]["PE"]["entry_trigger_price"],
...
def get_history():
    ...
    if not state:
        return jsonify({
            "state": {"is_active": False, "sl_hit_count": 0, "legs": {"CE": {}, "PE": {}}},
            "history": [],
        }), 200
