            ...
            if leg["status"] == "PENDING" and leg["entry_order_id"]:
                ...
                if st["status"] in FILLED_STATUSES:
                    ...
                    with state_lock:
                        ...
                        if not sl_order_id:
                            # ⚠️ ఇది చాలా ముఖ్యం — SL ఆర్డర్ పెట్టడం fail అయితే, position
                            # unprotected గా ఉండిపోతుంది (broker-side SL లేకుండా). ఇది strong
                            # గా alert చేస్తాం, ఎందుకంటే ఇక్కడే మనం ఇంతకుముందు మౌనంగా silent
                            # గా వదిలేసేవాళ్ళం — ఇది real financial risk.
                            state["trade_history"].append({
                                "leg": leg_name, "event": "SL_ORDER_FAILED",
                                "message": str(sl_res.get("message") or sl_res)[:200]
                                           + " — ⚠️ Position ప్రస్తుతం broker-side SL లేకుండా ఉంది, "
                                             "software trailing-SL మాత్రమే safety net గా పనిచేస్తోంది.",
                                "time": now_str,
                            })
