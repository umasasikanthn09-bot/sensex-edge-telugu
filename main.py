...
def get_running_day_1030_high(broker, symbol, access_token, api_key=""):
    ...
    Returns: (high, error_message_or_None)
    """
    broker = str(broker).lower()
    today_str = today_ist_str()
    ...
    if not window:
        return None, f"{symbol}: 10:30-10:44 window లో candles దొరకలేదు (మొత్తం {len(candles)} candles వచ్చాయి)."
...
def wait_for_running_day_1030_and_setup_legs(phone, session, state, ce_symbol, pe_symbol):
    ...
    while state["is_active"]:
        ...
        ce_high, ce_err = get_running_day_1030_high(broker, ce_symbol, access_token, api_key)
        pe_high, pe_err = get_running_day_1030_high(broker, pe_symbol, access_token, api_key)
