...
def _fetch_1min_candles(broker, instrument_id, access_token, api_key, date_str):
    ...
    ఏదైనా fail అయితే [] రిటర్న్ చేసి, exact HTTP status/response print చేస్తుంది.
    """
    broker = broker.lower()
    normalized = []
    if not instrument_id:
        print(f"[{broker.upper()} CANDLE] ❌ instrument identifier దొరకలేదు (resolve fail).")
        return []
    ...
    try:
        if broker == "upstox":
            ...
            if response.status_code != 200:
                print(f"[UPSTOX CANDLE] ❌ HTTP {response.status_code} | {str(res)[:400]}")
                return []
            ...
            res = response.json()
            if response.status_code != 200:
                print(f"[ZERODHA CANDLE] ❌ HTTP {response.status_code} | {str(res)[:400]}")
                return []
            for c in res.get("data", {}).get("candles", []):
            ...
            if response.status_code != 200 or res.get("status") is False:
                print(f"[ANGELONE CANDLE] ❌ HTTP {response.status_code} | {str(res)[:400]}")
                return []
            ...
            res = response.json()
            if response.status_code != 200:
                print(f"[DHAN CANDLE] ❌ HTTP {response.status_code} | {str(res)[:400]}")
                return []
            ...
            if response.status_code != 200 or res.get("s") != "ok":
                print(f"[FYERS CANDLE] ❌ HTTP {response.status_code} | {str(res)[:400]}")
                return []
            ...
            print(f"[WARNING] {broker} కోసం candle API ఇంకా ఇంప్లిమెంట్ కాలేదు.")

    except Exception as e:
        print(f"[{broker.upper()} CANDLE] ❌ Exception: {e}")
...
def fetch_915_high_low(broker, access_token, api_key=""):
    ...
    candles = _fetch_1min_candles(broker, instrument_id, access_token, api_key, today_str)
    if not candles:
    ...
    if not window:
        print(f"[{broker.upper()}] ⚠️ 09:15-09:29 window లో candles దొరకలేదు (మొత్తం {len(candles)} candles).")
        return None, None
...
def get_running_day_1030_high(broker, symbol, access_token, api_key=""):
    """
    ఈరోజు (running day) 10:30–10:44 విండో లో (5 బ్రోకర్లలో ఏదైనా) 1-min
    candles అగ్రిగేట్ చేసి High ఇస్తుంది. వెనకటి రోజు లాజిక్ లేదు.
    """
    broker = str(broker).lower()
    today_str = today_ist_str()
    ...
    high = max(c["high"] for c in window)
    print(f"[ALGO] 10:30 Candle ({broker}, {symbol}, {len(window)} bars) -> High:{high}")
    return high
...
def wait_for_915_data_and_setup_legs(phone, session, state):
    ...
    while state["is_active"]:
        ...
        high_915, low_915 = fetch_915_high_low(broker, access_token, api_key)
        ...
        if now_str >= DATA_WAIT_CUTOFF_TIME:
            with state_lock:
                ...
                state["trade_history"].append({
                    "leg": "-", "event": "ABORTED",
                    "message": f"మార్కెట్ ముగిసింది ({DATA_WAIT_CUTOFF_TIME}) — ఈరోజు 9:15 candle డేటా దొరకలేదు, స్ట్రాటజీ ఆగిపోయింది.",
                    "time": now_str,
                })
...
def wait_for_running_day_1030_and_setup_legs(phone, session, state, ce_symbol, pe_symbol):
    ...
    while state["is_active"]:
        ...
        ce_high = get_running_day_1030_high(broker, ce_symbol, access_token, api_key)
        pe_high = get_running_day_1030_high(broker, pe_symbol, access_token, api_key)
        ...
        if now_str >= ENTRY_REF_WAIT_CUTOFF_TIME:
            with state_lock:
                ...
                state["trade_history"].append({
                    "leg": "-", "event": "ABORTED",
                    "message": f"మార్కెట్ ముగిసింది ({ENTRY_REF_WAIT_CUTOFF_TIME}) — ఈరోజు 10:30 candle డేటా దొరకలేదు, ఎంట్రీ దొరకకుండానే స్ట్రాటజీ ఆగిపోయింది.",
                    "time": now_str,
                })
